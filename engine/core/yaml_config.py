...
class YAMLConfig(BaseConfig):
    ...
    def evaluator(self, ):
        if self._evaluator is None and 'evaluator' in self.yaml_cfg:
            ...
            elif self.yaml_cfg['evaluator']['type'] == 'DotaEvaluator':
                self._evaluator = create('evaluator', self.global_cfg, dataset=self.val_dataloader.dataset)
            elif self.yaml_cfg['evaluator']['type'] == 'DotaAutoEvaluator':
                self._evaluator = create('evaluator', self.global_cfg, dataset=self.val_dataloader.dataset)
            elif self.yaml_cfg['evaluator']['type'] == 'DotaAutoEvaluatorOffline':
            ...
            elif self.yaml_cfg['evaluator']['type'] == 'HRSCEvaluator':
                self._evaluator = create('evaluator', self.global_cfg, dataset=self.val_dataloader.dataset)
            else:
