...
class VGGBlock(nn.Module):
    def __init__(self, ch_in, ch_out, act='relu'):
        ...
        if ch_in == ch_out:
            self.identity = nn.BatchNorm2d(ch_in)
        else:
            self.identity = None
    ...
    def get_equivalent_kernel_bias(self):
        ...
        kernelid, biasid = self._fuse_bn_ident_tensor()
    ...
    def _fuse_bn_ident_tensor(self):
        if self.identity is None:
            return 0, 0

        input_dim = self.ch_in
        kernel_value = torch.zeros(self.ch_in, self.ch_in, 3, 3).to(self.identity.weight.device)
